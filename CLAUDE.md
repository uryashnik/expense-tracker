...
**Заголовок** — в формате коммита (`type(scope): описание`), из того же набора типов и scope. Это не косметика: мержим squash-мержем, поэтому заголовок PR становится сообщением коммита в `main` и обязан проходить commitlint. Проверяй до создания:

```bash
echo 'feat(web): add home dashboard with paginated transactions' | npx commitlint
```

**Тело пишем по фактическому диффу, а не по памяти.** Перед описанием смотрим, что реально изменилось:
...
Что должно быть в теле:

1. **Что и зачем** — одним абзацем, плюс что было до изменений.
2. **Изменения контракта** (API, `packages/shared`, схема БД) — фрагментом диффа, а не пересказом: ревьюер должен увидеть новую сигнатуру глазами.
3. **Решения, которых не видно из диффа** — почему сделано так, а не иначе. Это главная ценность описания: код показывает «что», а PR объясняет «почему».
4. **Попутно исправленные дефекты** — отдельным списком, чтобы их не искали в диффе фичи.
5. **Как проверено** — какие команды прогнаны и что проверено вживую. **И чего не проверяли** — умалчивать об этом нельзя.
6. **Что намеренно не вошло** и почему.
7. **Замечания для ревьюера** — сгенерированные файлы в диффе, требуемый порядок мержа, всё, обо что он споткнётся.

**Создание** — через `gh`, тело из файла, чтобы не терять разметку:

```bash
gh pr create --base main --head <branch> \
  --title 'feat(web): ...' --body-file <file>.md
```
