...
Токены компонентов (`--color-background`, `--color-primary`, `--color-border` и т.д.) в `globals.css` — не отдельная палитра, а алиасы через `var()` поверх уже существующих `--color-surface`/`--color-ink`/`--color-accent`: так тёмная тема остаётся общей и переопределяется в одном месте. Не заводи новый `--color-accent`-подобный токен под нейтральный hover-фон — там, где стандартный shadcn использует `accent`/`accent-foreground` (hover у `outline`/`ghost`), в этом проекте переиспользован `secondary`, потому что `--color-accent` уже занят под фирменный чернильный (= shadcn `primary`).

**Палитра монохромная.** `--color-accent` — чернильно-чёрный: им красятся кнопки, пилюли, квадраты иконок и активная страница пагинации. Цвет в интерфейсе несут только данные: пастельные плитки сводки (`--color-mint`, `--color-peach`), знак суммы (`--color-positive`) и цвета категорий, приходящие из API инлайн-стилем. Новый акцентный цвет «для красоты» не заводим.

В тёмной теме `--color-accent` **инвертируется** (становится почти белым), а вместе с ним `--color-on-accent` — чернильным: чёрная пилюля на тёмном холсте нечитаема. Ту же инверсию делает плитка остатка (`--color-night`): в светлой теме она тёмная среди пастельных, в тёмной — светлая. Правило: меняя эти токены, проверь **обе** темы, иначе акцент сольётся с фоном.

**Скругление кодирует роль элемента**, а не украшает: действия — `rounded-full` (кнопки, фильтры-пилюли), плиты и карточки — `rounded-3xl`/`rounded-[1.75rem]`, поля ввода и селекты внутри форм — `rounded-xl`, иконки — `rounded-2xl`. Полное скругление у поля формы во всю ширину смотрится чужеродно, поэтому `SelectTrigger` в диалогах переопределяется на `h-11 rounded-xl` — под высоту `Input`.

Тени — только `--shadow-plate` (плиты, всплывающие слои) и `--shadow-pill` (залитые кнопки). У пастельных плиток тени нет вовсе: их отделяет заливка, и одинаковая тень под каждым блоком стирает иерархию.

Фокус у кнопок — `outline` со смещением (`focus-visible:outline-2 outline-offset-2 outline-ink`), а не `ring`: кольцо цвета акцента на залитой акцентом пилюле невидимо, а `outline-ink` контрастен в обеих темах.
